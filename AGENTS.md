...
- 默认 inline CLI 使用 Codex 风格普通滚屏 composer，不启用 JLine `Status` 底部保留区：Windows Terminal 的缩放、全屏和标签恢复会重排 scrollback，即使单行 Status 也可能留下旧帧。任务运行时只维护一条瞬态活动行，用 `CR + CLEAR_LINE` 动态更新 spinner/耗时；不得上移光标、展示多行 reasoning 或改写历史 transcript。任务结束后追加稳定的 `Thought for Xs` 和工具卡片；实验性 dock 仅可通过 `-Dyuforge.inline.bottom-dock=true` 或 `YUFORGE_INLINE_BOTTOM_DOCK=true` 开启。
...
- ANSI 视觉层级必须按语义使用：正文保持终端默认前景色，`subtle` 用于仍需阅读的工具/角色摘要且不得 DIM，`muted` 只用于耗时、边框和 footer，`status/success/warning/error` 分别表示进行中、成功、警告和失败。Assistant 可见标签统一为 `YuForge:`，不要混用 `Assistant:`。
