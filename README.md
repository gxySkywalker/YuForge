...
- **Code Agent Runtime**：ReAct 为默认执行路径；复杂任务可切换 `/plan` 或 `/team`。Team Planner 会先用有界只读工具理解工作区，再输出 DAG 计划；格式异常时自动修复一次，写入与命令只交给 Worker。终端默认只展示阶段进度、步骤摘要、Worker 输出与工具证据，Planner/Reviewer 的内部 JSON 不进入正文。
