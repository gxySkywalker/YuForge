...
- **紧凑 Diff**：小改动直接显示彩色增删行，大改动折叠为 `Update(path) · +N -N`，需要时再用 `Ctrl+O` 展开。
- **可感知的工具进度**：构建、测试和命令执行期间按真实完成事件显示 `0/N → 1/N → N/N` 与动态耗时，完成后保留终态；`/team` 的并行步骤也持续显示 `completed/total`，Worker 详情仍按计划顺序输出；任务运行时可用 `Ctrl+O` 展开最近工具详情。
...
- 任务运行期使用单行动态 spinner 与耗时反馈；该活动行只重绘当前行，不上移光标或改写历史输出，兼顾 Windows Terminal 缩放稳定性。
- 终端状态和降级路径默认使用 ASCII 标签与 `>` 选择标记，避免 Windows 字体把 emoji/Dingbat 渲染成问号。
- inline transcript 默认真正折叠成功工具明细，仅让失败与拒绝突出显示；窄屏多列表格会自动降级为记录式布局，长路径不会被压成逐字符竖排。
- 同一轮任务的多次模型调用共享一个动态活动计时，结束后只保留一条累计 `Thought for`，避免工具往返产生重复计时噪音。
- 终端主题将正文、工具摘要、弱化元数据和成功/警告/失败状态分层着色；工具摘要保持可读，只有耗时、边框与 footer 使用 dim，回复标签统一为 `YuForge:`。
