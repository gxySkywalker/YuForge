...
import com.yuforge.context.TokenUsageFormatter;
...
public class Agent {
    ...
    private String buildProjectMemoryContext() {
        try {
            ...
            log.warn("Failed to load YUFORGE.md project memory", e);
            return "";
        }
    }
    ...
    public String getContextStatus() {
        com.yuforge.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.yuforge.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.yuforge.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.yuforge.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
