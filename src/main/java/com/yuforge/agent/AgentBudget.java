 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dyuforge.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dyuforge.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("yuforge.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("yuforge.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("yuforge.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
