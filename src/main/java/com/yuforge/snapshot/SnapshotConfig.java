...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("yuforge.snapshot.enabled", "YUFORGE_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("yuforge.snapshot.dir", "YUFORGE_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".yuforge", "snapshots").toString()));
        int max = readInt("yuforge.snapshot.max", "YUFORGE_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("yuforge.snapshot.excludes", "YUFORGE_SNAPSHOT_EXCLUDES", ""));
