...
import com.yuforge.render.inline.TerminalCapabilities;
 ...
 * <p>选型规则：
 * <ul>
 *   <li>{@code -Dyuforge.renderer} > {@code YUFORGE_RENDERER} 环境变量 > 默认 inline</li>
 ...
 *   <li>兼容：{@code YUFORGE_TUI=true} → 等价 {@code lanterna}，打 deprecation 提示</li>
 * </ul>
 *
...
public final class RendererFactory {
    ...
    public static Mode resolveMode() {
        ...
        // 兼容旧 YUFORGE_TUI=true → lanterna
        String legacyTui = System.getenv("YUFORGE_TUI");
        if (legacyTui != null && Boolean.parseBoolean(legacyTui.trim())) {
            System.err.println("[warn] YUFORGE_TUI=true 已废弃，请改用 YUFORGE_RENDERER=lanterna");
    ...
    private static Mode parse(String raw) {
        return switch (raw.trim().toLowerCase()) {
            ...
            default -> {
                System.err.println("[warn] 未识别的 YUFORGE_RENDERER='" + raw + "'，回退到 inline");
                yield Mode.INLINE;
            }
        };
    }
    ...
    public static Renderer create(Mode mode, Terminal terminal) {
        return switch (mode) {
            ...
            case INLINE -> {
                ...
                System.err.println("[warn] 终端不支持 ANSI，inline 模式回退到 plain");
